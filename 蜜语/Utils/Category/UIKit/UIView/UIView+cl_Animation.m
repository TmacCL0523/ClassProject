 ...
 *
 * 用户名:     hcl
 * 文件名:     UIView+cl_Animation.m
 * 创建时间:    2020-12-29
 *
 *********************************************************************************
 */

#import "UIView+cl_Animation.h"

float
    radiansForDegrees(int degrees) {
    return degrees * M_PI / 180;
}

@implementation UIView (cl_Animation)

#pragma mark - Moves

- (void)cl_moveTo:(CGPoint)destination duration:(float)secs option:(UIViewAnimationOptions)option {
    [self cl_moveTo:destination duration:secs option:option delegate:nil callback:nil];
}
...
- (void)cl_raceTo:(CGPoint)destination withSnapBack:(BOOL)withSnapBack {
    [self cl_raceTo:destination withSnapBack:withSnapBack delegate:nil callback:nil];
}
...
- (void)cl_spinCounterClockwise:(float)secs {
    [UIView animateWithDuration:secs / 4
        ...
        completion:^(BOOL finished) {
            [self cl_spinCounterClockwise:secs];
        }];
}
    ...
    [UIView animateWithDuration:secs / 2
        ...
        completion:^(BOOL finished) {
            [UIView animateWithDuration:secs / 2
                ...
                completion:^(BOOL finished) {
                    if( continuously ) {
                        [self cl_pulse:secs continuously:continuously];
                    }
                }];
        }];
}
